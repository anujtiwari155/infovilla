        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
                ...
                if($this->ion_auth->in_group('admin')) {
                    redirect(base_url().'backend/allfunction/dashboard', 'refresh');
                } elseif($this->ion_auth->in_group('vendor')) {
                    redirect(base_url().'vendor/Vendorfunction/vendor/get_started', 'refresh');
                } /*elseif($url) {
                    redirect(base_url().$url, 'refresh');
                } */elseif($this->ion_auth->in_group('customer')){
                    if(isset($this->session->model) && $this->session->model == 1) {
                        //print_r($this->session->url); die();
                ...
                redirect(base_url().'backend/Auth/login/'.$url, 'refresh');
                }
            } else {
                // if the login was un-successful
                // redirect them back to the login page
                $this->session->set_flashdata('message', $this->ion_auth->errors());
                redirect('backend/Auth/login/'.$url, 'refresh'); // use redirects instead of loading views for compatibility with MY_Controller libraries
            }
        } else {
            ...
            if($url) {
                if($url == 'vendor') {
                    ...
                    $this->data['model'] = 0;
                    $this->template->load('admin','auth/login', $this->data);
                }
